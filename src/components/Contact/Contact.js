...
const Contact = () => {
    ...
    const handeSend = (e) => {
        ...
        if (username === "") {
            ...
            setErrMsg("Please give your Email ID !");
        } else if (!emailValidation(email)) {
            setErrMsg("Give Valid Email ID!");
        } else if (subject === "") {
            setErrMsg("Please give your Subject !");
        } else if (message === "") {
            setErrMsg("Please give your Message !");
        } else {
            setSuccessMsg("Your Message has been send Successfully !");
            setErrMsg("");
            setUsername("");
            setPhoneNumber("");
            setEmail("");
        ...
        <section id='contact' className='w-full py-20 border-b-[1px] border-b-black'>
            ...
            <div className='w-full'>
                <div className='w-full h-auto flex flex-col lgl:flex-row justify-between'>
                    <ContactLeft />
                    <div className='w-full lgl:w-[60%] h-full py-10 bg-gradient-to-r from-[#1e2024] to-[#23272b] gap-8 p-4 lgl:p-8 rounded-lg shadow-shadowOne flex flex-col'>
                        <form className='w-full flex flex-col gap-6 py-2 lgl:py-5'>
                            ...
                            <div className='w-full flex flex-col lgl:flex-row gap-10'>
                                ...
                                <div className='w-full lgl:w-1/2 flex flex-col gap-4'>
                                    ...
                                    <input onChange={(e) => setPhoneNumber(e.target.value)}
                                        value={phoneNumber} className={`${errMsg === "Phone number is required !" &&
                                            "outline-designColor"
                            ...
                            <div className='flex flex-col gap-4'>
                                ...
                                <input onChange={(e) => setEmail(e.target.value)}
                                    value={email} className={`${errMsg === "Please give your Email ID !" &&
                                        "outline-designColor"
                                        } contactInput`} type='email' />
                            </div>
                            <div className='flex flex-col gap-4'>
                                <p className='text-sm text-gray-400 uppercase tracking-wide'>Subject</p>
                                <input onChange={(e) => setSubject(e.target.value)} value={subject} className={`${errMsg === "Please give your Subject !" &&
                                    "outline-designColor"
                                    } contactInput`} type='text' />
                            </div>
                            <div className='flex flex-col gap-4'>
                                <p className='text-sm text-gray-400 uppercase tracking-wide'>Your Message</p>
                                <textarea onChange={(e) => setMessage(e.target.value)} value={message} className={`${errMsg === "Please give your Message !" && "outline-designColor"
